...
`aigw` (AI Gateway) 是一个轻量级的多 Provider AI 网关，提供 OpenAI 兼容接口，支持负载均衡和多模态桥接。

## 特性

- **多 Provider 支持**：支持 iFlow、阿里云、智谱等多个 AI Provider
- **负载均衡**：支持 fastest（自动选最快）和 roundrobin（轮询）策略
- **协议兼容**：支持 OpenAI Chat Completions 和 Anthropic Messages API
- **模型别名**：通过 `provider/model` 格式指定上游 Provider
- **多模态桥接**：让不支持视觉的模型也能处理图片
- **流式响应**：完整支持 SSE 流式输出
...
官方 iFlow CLI 使用 Node.js 编写，选择 Node.js 实现 aigw 可以确保：

| 特征 | Node.js (aigw) | 其他语言 (Rust/Go) |
|------|----------------------|-------------------|
...
aigw 请求    → Node.js HTTP Client → TLS 指纹 A (相同)
...
```
┌─────────────┐     ┌───────────────┐     ┌─────────────┐     ┌────────────┐
│   Client    │────▶│  aigw  │────▶│  iFlow CLI  │────▶│  iFlow API │
...
```bash
git clone https://github.com/ryfineZ/aigw.git
cd aigw
npm install
```
